...
-- 6. جدول العقارات (Real Estate Listings)
CREATE TABLE IF NOT EXISTS real_estate_listings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    customer_id UUID REFERENCES customers(id) ON DELETE CASCADE,
    property_type VARCHAR(50) NOT NULL,
    listing_type VARCHAR(20) NOT NULL, -- 'sale' or 'rent'
...
CREATE INDEX IF NOT EXISTS idx_real_estate_customer ON real_estate_listings(customer_id);
CREATE INDEX IF NOT EXISTS idx_real_estate_published ON real_estate_listings(is_published);
CREATE INDEX IF NOT EXISTS idx_real_estate_type ON real_estate_listings(property_type);
...
GRANT ALL ON real_estate_listings TO anon, authenticated;
